...
namespace EhbEvoApp.Controllers
{
    public class OnderzoekController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var posts = db.Posts.Where(p => p.PostCategory == PostCategory.Onderzoek && p.UserId == currentUserId);
        ...
        public ActionResult Create([Bind(Include = "Id,Evopoints,UserId,PostType,PostCategory,Title,Content,ImgUrl,Date")] Post post)
        {
            if (ModelState.IsValid)
            {
                ...
                if(post.ImgUrl == null)
                {
                    ...
                    {
                        case PostType.Idee:
                            post.ImgUrl = Url.Content("~/Content/images/onderzoekidee.png");
                            break;
                        case PostType.Plan:
                            post.ImgUrl = Url.Content("~/Content/images/onderzoekplan.png");
                            break;
                        case PostType.Actie:
                            post.ImgUrl = Url.Content("~/Content/images/onderzoekactie.png");
                            break;
                        default:
                            break;
                    }
